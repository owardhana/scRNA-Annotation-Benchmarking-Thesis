...
# P8b -- Foundation Models (pretrained + fine-tuned) on Real datasets
...
BASE_DIR <- "./"
PLOT_DIR <- "./Plots"
DATA_CSV <- "./data/real_FM_results.tsv"

if (!dir.exists(PLOT_DIR)) dir.create(PLOT_DIR, recursive = TRUE)

# Dataset keys are derived from the CSV by stripping the trailing "_subset" tag.
SCENARIO_LABELS <- c(
...
# Continuous dataset profiles -- pull from Profiling_Real.csv and filter to the
# nine validation datasets. Keys must match the (suffix-stripped) sample column.
...
DATASET_PROFILES <- .profiling_raw %>%
  filter(Sample %in% names(SCENARIO_LABELS)) %>%
  transmute(
    scenario    = Sample,
    ...
    short_label = recode(Sample, !!!SCENARIO_LABELS)
  )
rm(.profiling_raw)
...
add_labels <- function(d) {
  d %>%
    mutate(
      scenario_label = factor(recode(scenario, !!!SCENARIO_LABELS),
...
# Loader: P8b CSV is one row per (model, sample). Strip the "_subset" suffix
# from sample names so they join cleanly to Profiling_Real.csv keys, then
# rename metric columns to the canonical *_mean names used downstream.
load_fm_flat <- function(csv_path) {
  raw <- read.delim(csv_path, stringsAsFactors = FALSE, check.names = FALSE)
  cat_map <- .build_category_map()
  combined <- raw %>%
    mutate(scenario = sub("_subset$", "", sample)) %>%
    dplyr::rename(
      tool                       = model,
      kappa_mean                 = kappa,
      f1_mean                    = macro_f1,
    ...
    mutate(
      replicate     = "rep1",
      tool_category = unname(cat_map[tool])
    )
  ...
  message("Loaded ", nrow(combined), " rows | ",
          length(unique(combined$scenario)), " dataset(s) | ",
...
df <- df %>%
  mutate(kappa_mean = ifelse(kappa_mean == 0 & accuracy == 0 & f1_mean == 0, NA_real_, kappa_mean))
...
# --- B. Friedman + Nemenyi (incomplete-tool filter excludes scBERT here) ---
...
# PLOT 1: Per-FM Cohen's κ per real dataset.
p1 <- df %>%
  add_labels() %>%
  filter(!is.na(tool_category)) %>%
  ...
  scale_fill_manual(values = MODEL_COLOURS, name = "Foundation Model", drop = FALSE) +
  scale_colour_manual(values = MODEL_COLOURS, name = "Foundation Model", drop = FALSE) +
  kappa_axis +
  labs(
    title    = "Cohen's κ by FM and real dataset (Real FM)",
    subtitle = "One panel per real dataset. One point per FM (single replicate per dataset).",
    x = "Foundation Model", y = "Mean Cohen's κ"
  ) +
  theme_bench() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8))

# PLOT 2: Heatmap of mean κ per (FM, dataset).
...
p2 <- heatmap_data %>%
  ...
  scale_fill_gradient2(low = "#c0392b", mid = "#f39c12", high = "#27ae60", midpoint = 0.5, limits = c(0, 1), breaks = c(0, 0.25, 0.5, 0.75, 1), name = "Cohen's \u03ba", na.value = "white",
                       guide = guide_colorbar(barwidth = unit(10, "cm"), barheight = unit(0.4, "cm"), title.position = "top", title.hjust = 0.5)) +
  labs(
    title    = "Per-FM Cohen's κ across real datasets (Real FM)",
    subtitle = "Each cell = mean κ. Empty cells = FM not run on that dataset.",
    x = "Dataset", y = "Foundation Model"
  ) +
  theme_bench() +
  theme(axis.text.y = element_text(size = 8), axis.text.x = element_text(angle = 45, hjust = 1, size = 9), strip.text.y = element_text(angle = 0, hjust = 0), panel.grid.major = element_blank())

# PLOT 3: Performance-runtime tradeoff.
tradeoff_data <- df %>%
  add_labels() %>%
...
p3 <- tradeoff_data %>%
  ...
  labs(
    title    = "Performance–runtime tradeoff (Real FM)",
    subtitle = "X-axis log₁₀ scale. Each point = one FM, averaged across all real datasets.",
...
stability_data <- df_scenario_mean %>%
  ...
  filter(!is.na(kappa_drop), is.finite(kappa_drop)) %>%
...
p5 <- stability_data %>%
  ...
  labs(
    title    = "κ stability across real datasets (Real FM)",
    subtitle = "Bar height = max κ − min κ across datasets.",
    x = NULL, y = "κ range (best − worst)"
  ) +
  theme_bench() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 9))

# PLOT 6: Macro F1 vs Cohen's κ.
agreement_data <- df %>%
  add_labels() %>%
  filter(!is.na(tool_category)) %>%
  group_by(tool, tool_category) %>%
...
p7 <- df %>%
  ...
  scale_y_log10(labels = label_number(suffix = "s", big.mark = ",")) +
  labs(
    title    = "Runtime by FM and real dataset (Real FM)",
...
# PLOT 8: Peak GPU memory distribution per FM across datasets.
p8 <- df %>%
  add_labels() %>%
  filter(!is.na(tool_category), !is.na(peak_system_memory_mean_mb), peak_system_memory_mean_mb > 0) %>%
  ...
  labs(
    title    = "Peak GPU memory by FM and real dataset (Real FM)",
    subtitle = "Y-axis log₁₀ scale.",
...
p10 <- df %>%
  ...
  scale_y_continuous(limits = c(0, 1), labels = percent_format(accuracy = 1)) +
  labs(
    title    = "Rare cell-type F1 by FM and real dataset (Real FM)",
...
p13 <- eta_sq_df %>%
  ...
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  labs(
    title    = "Dataset factor decomposition — Cohen’s κ variance (Real FM)",
...
p13b <- eta_sq_rt_df %>%
  ...
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  labs(
    title    = "Dataset factor decomposition — Runtime variance (Real FM)",
...
# PLOT 14: Mean κ vs each dataset characteristic.
...
p14 <- (sc_N | sc_I) / (sc_K | sc_D) +
  plot_annotation(
    title    = "Dataset characteristic scatter plots (Real FM)",
    subtitle = "Each point = one real dataset. Mean Cohen’s κ across all FMs. Linear trend ± 95% CI.",
    theme    = theme_bench()
  )
...
p33 <- (sc_N33 | sc_I33) / (sc_K33 | sc_D33) +
  plot_annotation(
    title    = "Dataset characteristics vs runtime (Real FM)",
    subtitle = "Each point = one real dataset. Mean runtime across all FMs. Y-axis log₁₀ scale.",
    theme    = theme_bench()
  )

# PLOT 34/35/36: Category-stratified diagnostics (single category here).
...
p34 <- if (!is.null(eta_rt_by_category) && nrow(eta_rt_by_category) > 0) {
  eta_rt_by_category %>%
    ...
    scale_y_continuous(labels = percent_format(accuracy = 1)) +
    labs(
      title    = "Category-stratified factor sensitivity — Runtime (η²) (Real FM)",
...
p35 <- if (!is.null(coef_rt_by_category) && nrow(coef_rt_by_category) > 0) {
  coef_rt_by_category %>%
    ...
    labs(
      title    = "Effect of dataset characteristics on runtime (Real FM)",
...
p36 <- if (!is.null(coef_kappa_by_category) && nrow(coef_kappa_by_category) > 0) {
  coef_kappa_by_category %>%
    ...
    scale_colour_manual(values = CATEGORY_COLOURS, name = "Tool Category") +
    labs(
      title    = "Effect of dataset characteristics on Cohen's κ (Real FM)",
...
# PLOT 26: Per-category scatter (single category here).
...
p26 <- if (length(cat_plots_26) > 0) {
  purrr::reduce(cat_plots_26, `/`) +
    plot_annotation(
      title    = "Dataset characteristic scatter plots — by category (Real FM)",
      subtitle = "Each point = one real dataset.",
      theme    = theme_bench()
    )
} else NULL
...
p28 <- (sc_N28 | sc_I28) / (sc_K28 | sc_D28) +
  ...
  plot_annotation(
    title    = "Dataset characteristic plots — by category (Real FM)",
    subtitle = "Mean Cohen’s κ per (dataset, category).",
    theme    = theme_bench()
  ) &
...
p29 <- (sc_N29 | sc_I29) / (sc_K29 | sc_D29) +
  plot_layout(guides = "collect") +
  plot_annotation(
    title    = "Individual FM characteristic response — Cohen's κ (Real FM)",
...
p30 <- (sc_N30 | sc_I30) / (sc_K30 | sc_D30) +
  plot_layout(guides = "collect") +
  plot_annotation(
    title    = "Runtime characteristic response — by category (Real FM)",
    subtitle = "Mean runtime per (dataset, category). Y-axis log₁₀ scale.",
    theme    = theme_bench()
  ) &
  theme(legend.position = "bottom")

# PLOT 27: With one replicate per dataset, per-dataset Friedman is undefined.
# Skip generation; emit a message.
p27 <- NULL
...
p15 <- mean_ranks_df %>%
  ...
  labs(
    title    = "Critical difference diagram — Nemenyi post-hoc (Real FM)",
    subtitle = sprintf("Tools joined by a bar are not significantly different (α = 0.05; CD = %.2f, k = %d, N = %d datasets)%s",
                       CD, k_tools, N_scenarios,
                       if (length(setdiff(fm_models, complete_tools)) > 0)
                         paste0(". ", paste(setdiff(fm_models, complete_tools), collapse = ", "), " excluded — incomplete coverage.")
                       else "."),
    y = NULL
  ) +
  theme_bench() +
...
rank_heatmap_df <- as.data.frame(rank_mat) %>%
  ...
  mutate(
    tool           = factor(tool, levels = rev(complete_tools)),
...
p16 <- rank_heatmap_df %>%
  ...
  labs(
    title    = "FM rank within each real dataset (Real FM)",
    subtitle = if (length(setdiff(fm_models, complete_tools)) > 0)
      paste0("Rank 1 = highest κ on that dataset. ", paste(setdiff(fm_models, complete_tools), collapse = ", "), " excluded — incomplete coverage.")
    else "Rank 1 = highest κ on that dataset.",
    x = "Dataset", y = "Foundation Model"
  ) +
  theme_bench() +
...
p18 <- p18_data %>%
  ...
  scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 1)) +
  labs(
    title    = "Classification difficulty vs. category performance (Real FM)",
...
p19 <- corr_long %>%
  ...
  labs(
    title    = "Metric correlation matrix (Real FM)",
    subtitle = "Spearman ρ between per-FM mean metrics across all datasets.",
    x = NULL, y = NULL
  ) +
  theme_bench() +
  theme(axis.text.x = element_text(angle = 30, hjust = 0), panel.grid = element_blank())

# PLOT 20: Ridge plots per dataset.
p20 <- df_scenario_mean %>%
  ...
  labs(
    title    = "Distribution of Cohen's κ by real dataset (Real FM)",
    subtitle = "Datasets ordered by difficulty (easiest at top). Density across the available FMs.",
    x = "Mean Cohen's κ", y = NULL
  ) +
  theme_bench()

# PLOT 21: Top 3 FMs per dataset.
...
p21 <- best_tool_per_scenario %>%
  ...
  labs(
    title    = "Top 3 FMs per real dataset (Real FM)",
...
p22 <- cliff_data %>%
  ...
  scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 1)) +
  labs(
    title    = "Performance cliff — κ range per FM (Real FM)",
    subtitle = "▲ = best dataset, ▼ = worst dataset, ● = mean.",
...
if (!is.null(p_mat)) {
  ...
  p23 <- pmat_long %>%
    ...
    labs(
      title    = "Nemenyi post-hoc p-value matrix (Real FM)",
      subtitle = "Both axes sorted by mean rank. scBERT excluded — incomplete coverage.",
      x = "Tool A", y = "Tool B"
    ) +
    theme_bench() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 9),
...
# PLOT 25: Parallel coordinates -- replace coverage with accuracy.
...
p25 <- parallel_data %>%
  ...
  labs(
    title    = sprintf("Multi-metric profile — %d FMs (Real FM)", length(top_tools)),
...
# PLOT 31: KNN purity vs κ per category (single panel here).
...
p31 <- patchwork::wrap_plots(cat_knn_plots_31, ncol = 1) +
  plot_annotation(
    title    = "KNN Purity vs Mean Cohen’s κ — by category (Real FM)",
    subtitle = "Each point = one real dataset.",
    theme    = theme_bench()
  )
...
p32 <- ggplot(resid_corr_32, aes(x = characteristic, y = tool_category, fill = rho)) +
  ...
  labs(
    title    = "Residual correlations after regressing out KNN Purity (Real FM)",
...
ggsave("plot26_main_effects_by_category.png",    p26, width = 305, height = 203, units = "mm", dpi = 600, path = PLOT_DIR)
...
message("\n", bar)
message("  STATISTICAL SUMMARY — P8b Foundation Models (Real)")
message(bar)
...
message(sprintf("  FMs included in Friedman analysis: %s",
                paste(complete_tools, collapse = ", ")))
message(sprintf("  FMs excluded (incomplete coverage): %s",
                paste(setdiff(fm_models, complete_tools), collapse = ", ")))
