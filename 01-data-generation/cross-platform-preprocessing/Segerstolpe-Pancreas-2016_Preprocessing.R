...
# 1. Load and Convert
# ==============================================================================
# This pulls the RDS directly into your R environment
sce <- SegerstolpePancreasData()
dim(sce)

# Print assays for verification
assayNames(sce)
count_mat <- assay(sce, "counts")
meta <- as.data.frame(colData(sce))

# Check duplicates in the matrix itself
cat("Duplicated genes in count_mat:", sum(duplicated(rownames(count_mat))), "\n")

# See which genes are duplicated
dup_genes <- rownames(count_mat)[duplicated(rownames(count_mat))]
cat("Example duplicates:", head(dup_genes), "\n")

# Fix: keep only first occurrence of each gene
count_mat <- count_mat[!duplicated(rownames(count_mat)), ]

# Also replace underscores now to avoid Seurat warning
# (Seurat does this automatically but doing it explicitly avoids confusion)
...
cell_type_column <- "cell.type"  # Update this if the column name is different

message("Actual cells in original: ", ncol(seurat_obj))

table(seurat_obj@meta.data[[cell_type_column]])

keep_types <- c("acinar cell", "alpha cell", "beta cell", "delta cell", "ductal cell", "endothelial cell", "epsilon cell", "gamma cell", "mast cell")

cells_to_keep <- which(!is.na(seurat_obj[[cell_type_column]]) & 
                         seurat_obj[[cell_type_column, drop = TRUE]] %in% keep_types)

seurat_obj <- subset(seurat_obj, cells = cells_to_keep)

# Removes " cell" (including the leading space) to match Baron
seurat_obj[[cell_type_column]] <- gsub(" cell", "", seurat_obj[[cell_type_column, drop = TRUE]])

# Verify the change
table(seurat_obj[[cell_type_column]])

message("Actual cells in filtered: ", ncol(seurat_obj))
