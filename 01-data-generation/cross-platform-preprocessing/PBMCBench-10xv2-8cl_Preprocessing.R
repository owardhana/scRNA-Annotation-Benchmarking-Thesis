...
# 1. Load and Convert
# ==============================================================================
seurat_obj <- readRDS('PBMCBench_Seurat.rds')
...
table(seurat_obj@meta.data$Method, seurat_obj@meta.data$Ground_Truth_Celltype)

keep_types <- c("B cell", "CD14+ monocyte", "CD16+ monocyte", "CD4+ T cell", "Cytotoxic T cell", "Dendritic cell", "Natural killer cell", "Plasmacytoid dendritic cell")
target_method <- "10x Chromium (v2)"

# Single step filtering: No NAs, specific types, AND specific method
cells_to_keep <- which(!is.na(seurat_obj[[cell_type_column]]) & 
                         seurat_obj[[cell_type_column, drop = TRUE]] %in% keep_types &
                         seurat_obj[["Method", drop = TRUE]] == target_method)
