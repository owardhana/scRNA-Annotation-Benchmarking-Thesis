...
# ==============================================================================
# 1. Load and Convert + HGNC Mapping
# ==============================================================================
# This pulls the RDS directly into your R environment
sce <- fetchDataset("zhao-immune-2020", '2023-12-22', path = NA, realize.assays=TRUE)
...
# 2. Query for the latest Human EnsDb (Ensembl database)
# This finds a database that is guaranteed to match your IDs
q <- query(ah, c("EnsDb", "Hsapiens", "GRCh38"))
edb <- q[[length(q)]] # Selects the most recent version

# 3. Create a map using the EnsDb object instead of org.Hs.eg.db
# We map from "GENEID" (the keytype for Ensembl) to "SYMBOL"
gene_map_vector <- mapIds(
  edb,
  keys = clean_ids,
  column = "SYMBOL",
  keytype = "GENEID",
...
# 4. Integrate into your original names
final_names <- rownames(sce)
final_names[is_ensembl] <- ifelse(is.na(gene_map_vector), ensembl_rows, gene_map_vector)
rownames(sce) <- make.unique(final_names)
...
cell_type_column <- "fine"  # Update this if the column name is different
