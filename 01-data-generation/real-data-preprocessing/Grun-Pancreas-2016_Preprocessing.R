...
sce <- fetchDataset("grun-pancreas-2016", '2023-12-14', path = NA, realize.assays=TRUE)
...
# Clean the gene names (rownames of the matrix)
new_gene_names <- rownames(count_mat)
new_gene_names <- sub("__.*$", "", new_gene_names)

# Update rownames
rownames(count_mat) <- new_gene_names

# Handle duplicates (Crucial!)
...
cell_type_column <- "sample"  # Update this if the column name is different
