...
# ==============================================================================
# 3. Create the Subset (First 5 Clusters)
# ==============================================================================
# Get all unique clusters in the dataset
unique_clusters <- unique(seurat_obj@meta.data[[cell_type_column]])

# Keep only the first 5 unique clusters
target_clusters <- head(unique_clusters, 5)
message("\nSubsetting to keep only the following 5 clusters: \n  - ", paste(target_clusters, collapse = "\n  - "))

# Identify all cell barcodes that belong to these 5 clusters
...
message("\nActual cells exported for use (5 clusters only): ", ncol(seurat_subset))
