...
assayNames(sce_sc_10x_5cl_qc)

seurat_obj <- as.Seurat(sce_sc_10x_5cl_qc, counts = "counts", data = "logcounts")

# FIND OUT THE NAME OF THE METADATA COLUMN THAT HAS THE CELL TYPE INFO
head(seurat_obj@meta.data)
cell_type_column <- "cell_line_demuxlet"  # Update this if the column name is different
...
metadata_orig <- seurat_obj@meta.data %>%
  tibble::rownames_to_column("barcode") %>%
  select(barcode, Ground_Truth_Celltype = all_of(cell_type_column))
...
metadata_subset <- seurat_subset@meta.data %>%
  tibble::rownames_to_column("barcode") %>%
  select(barcode, Ground_Truth_Celltype = all_of(cell_type_column))
