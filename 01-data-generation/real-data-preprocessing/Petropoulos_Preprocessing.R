...
counts_dt <- fread("Petropoulos_2016_PMID_27062923.counts.gz")
counts_matrix <- as.matrix(counts_dt[, -1], rownames = counts_dt[[1]])
seurat_obj <- CreateSeuratObject(counts = counts_matrix)

# Load the etadata
metadata <- read.table("Petropoulos_2016_PMID_27062923.meta.tsv", header = TRUE, sep = "\t", row.names = 1)
...
cell_type_column <- "raw_annotation"  # Update this if the column name is different
