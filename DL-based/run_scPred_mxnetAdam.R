...
#'
#' Purpose: Run scPred algorithm using deep learning-based classification
#' Inputs:
...
#'            MXNet neural network with Adam optimizer via mxnet package
run_scPred_mxnetAdam_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("bench", quietly = TRUE)) {
    ...
    seurat_train <- trainModel(seurat_train, model = "mxnetAdam", allowParallel = TRUE)
  ...
  return(list(
    ...
    warning(paste("scPred (mxnetAdam) error:", e$message))
    return(default_return())
  })
}

# For backward compatibility
run_scPred_mxnetAdam <- run_scPred_mxnetAdam_function
