SampleNorm <- function(train_count,
  ...
  cmax <- apply(train_count, 2, max, na.rm = TRUE)
  cmin <- apply(train_count, 2, min, na.rm = TRUE)
  range_vals <- cmax - cmin
  range_vals[is.na(range_vals) | range_vals == 0] <- 1
  tmp1 <- sweep(train_count, 2, cmin, "-")
  tmp2 <- sweep(tmp1, 2, range_vals, "/")
  tmp2[!is.finite(tmp2)] <- 0

  ### let's try some feature selection here
  nfeature <- min(nfeature, nrow(tmp2))
  oosd <- apply(tmp2, 1, sd, na.rm = TRUE)
  ...
  if(!is.null(test_count)) {
    ### sample normalization
    cmax <- apply(test_count, 2, max, na.rm = TRUE)
    cmin <- apply(test_count, 2, min, na.rm = TRUE)
    range_vals <- cmax - cmin
    range_vals[is.na(range_vals) | range_vals == 0] <- 1
...
#  require(keras)
  ...
  val_split <- if (nrow(train_count) >= 10) 0.2 else 0
  fit_args <- list(
    x = train_count, y = train_label,
    epochs = 15, batch_size = 256,
    validation_split = val_split,
    verbose = verbose, return_best_model = TRUE
  )
  do.call(fit, c(list(object = model_big), fit_args))
...
#  require(keras)
  ...
  if (val_split > 0) {
    fit_args$callbacks <- list(
      callback_early_stopping(patience = 10),
      callback_reduce_lr_on_plateau(factor = 0.05)
    )
  }
  do.call(fit, c(list(object = model_medium), fit_args))
...
#  require(keras)
  ...
  do.call(fit, c(list(object = model_small), fit_args))
