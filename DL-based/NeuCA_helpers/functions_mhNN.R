...
GetFeature <- function(LeftRef, RightRef, nMark) {
  # Limma replaced with simple t-test to avoid statmod corruption
  # Finds top nMark genes that differ between LeftRef and RightRef
  ...
  # Handle case where no genes pass filter
  if (is.null(nrow(allRef2)) || nrow(allRef2) == 0) {
    return(rownames(allRef)[1:min(nMark, nrow(allRef))])
  }

  # Calculate t-statistic for each gene
  n_left <- ncol(LeftRef)
  n_right <- ncol(RightRef)

  t_stats <- apply(allRef2, 1, function(gene_expr) {
    left_vals <- gene_expr[1:n_left]
    right_vals <- gene_expr[(n_left + 1):(n_left + n_right)]

    # Simple t-test calculation (avoids external packages)
    mean_diff <- mean(left_vals) - mean(right_vals)

    # Pooled variance
    var_left <- var(left_vals)
    var_right <- var(right_vals)
    pooled_var <- ((n_left - 1) * var_left + (n_right - 1) * var_right) /
                  (n_left + n_right - 2)

    # Standard error
    se <- sqrt(pooled_var * (1/n_left + 1/n_right))

    # t-statistic (absolute value for ranking)
  ...
  # Rank genes by absolute t-statistic (highest = most differential)
  ranked_genes <- names(sort(t_stats, decreasing = TRUE))

  # Return top nMark genes
  return(ranked_genes[1:min(nMark, length(ranked_genes))])
}
...
train_SCH <- function(train_count, train_lab,
  ...
  for(i in seq_along(uniqlab)) {
    grpmean[,i] <- rowMeans(train_count[, which(train_lab == uniqlab[i]), drop = FALSE])
  }
  ...
  for(iter in 1:nrow(treelabels)) {
    ...
    LeftRef <- train_count[, train_lab %in% treeCellTypes[[iter]][[1]], drop = FALSE]
    RightRef <- train_count[, train_lab %in% treeCellTypes[[iter]][[2]], drop = FALSE]
    if (ncol(LeftRef) == 0 || ncol(RightRef) == 0) {
      warning("Skipping hierarchical NN step ", iter, ": empty cell group")
      HNNtrees[[iter]] <- NULL
      next
    }
...
SCHwrapper <- function(train_count, train_label,
  ...
  train_max <- max(train_count, na.rm = TRUE)
  if (!is.finite(train_max) || train_max == 0) train_max <- 1
  train_count_normed <- train_count / train_max
  test_max <- max(test_count, na.rm = TRUE)
  if (!is.finite(test_max) || test_max == 0) test_max <- 1
  test_count_normed <- test_count / test_max
  oosd <- apply(train_count_normed, 1, sd, na.rm = TRUE)
  n_select <- min(5000, nrow(train_count_normed))
  train_count_normed <- train_count_normed[order(oosd, decreasing = TRUE)[1:n_select], ]
