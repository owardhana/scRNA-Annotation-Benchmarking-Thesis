...
cor.det = function(dat, lb){
    ...
    for(i in seq_along(lname)){
        ...
        cge = ct[, which(lb == lname[i]), drop = FALSE]
        #average the profiles
        tmp = rowMeans(cge, na.rm = TRUE)
        #store them in mp.ge
        mp.ge[,i] = tmp
    }
    ac = cor(mp.ge, use = "pairwise.complete.obs")
    diag(ac) = 0
    #return 1 if all cor < 0.95, return 2 of at least 1 cor >= 0.95
    # Handle NAs: treat NA correlations as not meeting the 0.95 threshold
    cd = ifelse(sum(ac < 0.95, na.rm = TRUE) == sum(!is.na(ac)), 1, 2)
    return(cd)
}
