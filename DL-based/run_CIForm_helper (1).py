...
# Set device and tensor type based on availability
# Force CPU mode due to RTX 5060 sm_120 incompatibility with PyTorch 2.3.1
device = torch.device('cpu')
torch.set_default_tensor_type(torch.DoubleTensor)
print("Using CPU (RTX 5060 sm_120 incompatible with PyTorch 2.3.1)")
...
def ciForm(s ,Train_adata ,train_labels ,Test_adata,n_epochs=20):
    ...
    labels = Train_labels
    # CRITICAL FIX: Do NOT overwrite cell_types with np.unique(train_labels)
    # This was causing label mismatch between encoding and decoding
    num_classes = len(train_cellTypes) + 1

    query_data = getXY(gap, Test_adata, [])

    model = CIForm(input_dim=d_models, nhead=heads, d_model=d_models,
                   num_classes=num_classes, dropout=dp).to(device)
    ...
    # Debug: Print label mapping to verify correctness
    print(f"\n=== Label Mapping ===")
    print(f"Encoding used for training:")
    for idx, ct in enumerate(cell_types):
        print(f"  {ct} → {idx + 1}")
    print(f"Decoding will use:")
    for idx, ct in enumerate(new_cellTypes):
        print(f"  {idx} → {ct}")
    ...
    # Map numerical predictions to cell type names
    mapped_predictions = [new_cellTypes[pred] if pred < len(new_cellTypes) else "unassigned"
                         for pred in y_predict]

    # Print prediction statistics
    from collections import Counter
    pred_distribution = Counter(mapped_predictions)
    print(f"\n=== CIForm Prediction Statistics ===")
    print(f"Total predictions: {len(mapped_predictions)}")
    print(f"Prediction distribution:")
    for celltype, count in sorted(pred_distribution.items()):
        pct = 100 * count / len(mapped_predictions)
