...
run_scLearn_function <- function(seurat_train, seurat_test, markers) {
  ...
  tryCatch({
    ...
    min_cells_per_type <- 3  # Minimum for CV folds
    ...
    if (!requireNamespace("bench", quietly = TRUE)) {
      ...
      # Step 2: Cell type filtering (remove rare cell types)
      ...
      if (length(remaining_cell_types) < 2) {
        warning("Too few cell types remaining after filtering")
        return(default_return())
      }

      # Step 3: Feature selection using M3Drop
      ...
      # Step 4: Model training with scLearn
      ...
      # Step 5: Quality control for test data
      ...
      bench_result <- bench::mark(
        {
          ...
          remaining_cell_types <- unique(data_type_filtered$sample_information_cellType)
          if (length(remaining_cell_types) < 2) {
            stop("Too few cell types remaining after filtering")
          }
    ...
    # Handle early termination from bench::mark block
    if (!exists("scLearn_predict_result")) {
      warning("Too few cell types remaining after filtering")
      return(default_return())
    }
