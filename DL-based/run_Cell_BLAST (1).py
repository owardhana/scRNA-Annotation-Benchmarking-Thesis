...
try:
    ...
    def _run_method():
            ...
            try:
                if cell_id in annotation_df.index:
                    ...
                    if pred in ['rejected', 'ambiguous']:
                        predictions.append("Unknown"); confidences.append(0.0)
                    else:
                        predictions.append(str(pred))
                        maj_frac = annotation_df.loc[cell_id, 'majority_frac'] if 'majority_frac' in annotation_df.columns else None
                        confidences.append(float(maj_frac) if maj_frac is not None and not pd.isna(maj_frac) else 0.8)
                else:
                    predictions.append("Unknown"); confidences.append(0.0)
            except Exception as ex:
                print(f"WARNING: Failed to extract prediction for cell {{cell_id}}: {{ex}}")
                predictions.append("Unknown"); confidences.append(0.0)
        ...
        return predictions, confidences, list(adata_test.obs['Ground_Truth_Celltype']), cell_ids, _peak_vram_mb

    mem_list, (predictions, confidences, true_labels, cell_ids, peak_vram_mb) = memory_usage(
