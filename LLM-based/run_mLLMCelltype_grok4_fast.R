# run_mLLMCelltype_grok4_fast.R
# Single-model variant using Grok 4 Fast via OpenRouter

run_mLLMCelltype_grok4_fast_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("bench", quietly = TRUE)) {
    ...
    annotation_results <- tryCatch({
      cat("Calling annotate_cell_types with Grok 4 Fast...\n")
      ...
      warning("mLLMCelltype Grok 4 Fast annotation failed: ", e$message)
    ...
    if (is.null(annotation_results)) {
      warning("mLLMCelltype Grok 4 Fast failed - returning default predictions")
      return(default_return())
    }
...
run_mLLMCelltype_grok4_fast <- run_mLLMCelltype_grok4_fast_function
