# run_mLLMCelltype_qwen3_max.R
# Single-model variant using Qwen 3 Max

run_mLLMCelltype_qwen3_max_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    ...
    annotation_results <- tryCatch({
      cat("Calling annotate_cell_types with Qwen 3 Max...\n")
      ...
      warning("mLLMCelltype Qwen 3 Max annotation failed: ", e$message)
    ...
    if (is.null(annotation_results)) {
      warning("mLLMCelltype Qwen 3 Max failed - returning default predictions")
      return(default_return())
    }
...
run_mLLMCelltype_qwen3_max <- run_mLLMCelltype_qwen3_max_function
