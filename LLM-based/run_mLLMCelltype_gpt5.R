# run_mLLMCelltype_gpt5.R
# Single-model variant using GPT-5

run_mLLMCelltype_gpt5_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (nchar(api_key) == 0) {
    warning("OPENAI_API_KEY not found - mLLMCelltype_gpt5 requires API key")
  ...
  if (!requireNamespace("bench", quietly = TRUE)) {
    ...
    annotation_results <- tryCatch({
      cat("Calling annotate_cell_types with GPT-5...\n")
      ...
      warning("mLLMCelltype GPT-5 annotation failed: ", e$message)
    ...
    if (is.null(annotation_results)) {
      warning("mLLMCelltype GPT-5 failed - returning default predictions")
      return(default_return())
    }
...
run_mLLMCelltype_gpt5 <- run_mLLMCelltype_gpt5_function
