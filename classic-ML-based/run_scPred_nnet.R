...
#'            Single-hidden-layer neural network via nnet package
run_scPred_nnet_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("bench", quietly = TRUE)) {
    ...
    seurat_train <- trainModel(seurat_train, model = "nnet", allowParallel = TRUE)
  ...
  return(list(
    ...
    warning(paste("scPred (nnet) error:", e$message))
    return(default_return())
  })
}

# For backward compatibility
run_scPred_nnet <- run_scPred_nnet_function
