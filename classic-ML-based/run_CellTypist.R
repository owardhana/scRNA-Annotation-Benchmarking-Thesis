...
run_CellTypist_function <- function(seurat_train, seurat_test, markers) {
  ...
  tryCatch({
    ...
    # Parse peak memory from Python output
    peak_memory_mb <- NA
    if (!is.null(python_output)) {
      memory_line <- grep("PEAK_MEMORY_MB:", python_output, value = TRUE)
      if (length(memory_line) > 0) {
        peak_memory_mb <- as.numeric(sub(".*PEAK_MEMORY_MB:([0-9.]+).*", "\\1", memory_line[1]))
        message(sprintf("Peak memory usage: %.2f MB", peak_memory_mb))
      }
    }
