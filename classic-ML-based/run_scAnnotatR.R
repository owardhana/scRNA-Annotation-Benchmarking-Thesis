    ...
    if (!requireNamespace("bench", quietly = TRUE)) {
      ...
      for (ct in unique_celltypes) {
        tryCatch({
          ...
          ct_markers <- markers[markers$cluster == ct & markers$avg_log2FC > 0, "gene"]
          
          # Intersect markers with genes present in the SCE object
          available_genes <- intersect(ct_markers, rownames(sce_train))
          
          # Train if we have enough markers
          if(length(available_genes) >= 5) {
            
            # Native SCE training call
            classifier <- train_classifier(
              train_obj = sce_train,
              assay = "counts",           # Use 'counts' assay from SCE
              cell_type = ct,
              marker_genes = head(available_genes, 100),
          ...
          # message(paste("Skipping", ct, ":", e$message))
        })
      }
      ...
      # Classify using the converted SCE test object
      sce_test_classified <- classify_cells(
      ...
      # Need to re-run to get results since bench::mark doesn't return them
      classifiers <- list()
