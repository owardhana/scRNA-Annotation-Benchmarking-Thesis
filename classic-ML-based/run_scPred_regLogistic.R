...
#'            Regularized logistic regression via LiblineaR package
run_scPred_regLogistic_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    ...
    seurat_train <- trainModel(seurat_train, model = "regLogistic", allowParallel = TRUE)
  ...
  return(list(
    ...
    warning(paste("scPred (regLogistic) error:", e$message))
    return(default_return())
  })
}

# For backward compatibility
run_scPred_regLogistic <- run_scPred_regLogistic_function
