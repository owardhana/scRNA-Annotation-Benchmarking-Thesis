...
run_scPred_lda_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("bench", quietly = TRUE)) {
    ...
    seurat_train <- trainModel(seurat_train, model = "lda", allowParallel = TRUE)
  ...
  return(list(
    ...
    warning(paste("scPred (lda) error:", e$message))
    return(default_return())
  })
}

# For backward compatibility
run_scPred_lda <- run_scPred_lda_function
