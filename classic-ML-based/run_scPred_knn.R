...
run_scPred_knn_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    ...
    seurat_train <- trainModel(seurat_train, model = "knn", allowParallel = TRUE)
  ...
  return(list(
    ...
    warning(paste("scPred (knn) error:", e$message))
    return(default_return())
  })
}

# For backward compatibility
run_scPred_knn <- run_scPred_knn_function
