...
#'            Multilayer Perceptron via RSNNS package
run_scPred_mlp_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("bench", quietly = TRUE)) {
    ...
    seurat_train <- trainModel(seurat_train, model = "mlp", allowParallel = TRUE)
  ...
  return(list(
    ...
    warning(paste("scPred (mlp) error:", e$message))
    return(default_return())
  })
}

# For backward compatibility
run_scPred_mlp <- run_scPred_mlp_function
