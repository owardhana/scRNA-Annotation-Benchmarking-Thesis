...
def main():
    ...
    try:
        ...
        if _HAS_MEMORY_PROFILER:
            mem_samples = memory_usage(run_core_algorithm, interval=0.05)
            peak_system_memory_mb = max(mem_samples)
        else:
            run_core_algorithm()
        runtime_secs = time.perf_counter() - perf_start

        print(f"RUNTIME_SECS:{runtime_secs:.4f}")
        print(f"PEAK_SYSTEM_MEMORY_MB:{peak_system_memory_mb:.2f}")
