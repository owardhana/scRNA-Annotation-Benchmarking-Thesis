...
run_CALLR_function <- function(seurat_train, seurat_test, markers) {
  ...
  tryCatch({
    ...
    if (!requireNamespace("bench", quietly = TRUE)) {
      ...
      library(bench)
      bench_result_preprocess <- bench::mark(
        {
          # Preprocess the combined data
          processed_data <- preprocess(combined_counts)
        },
      ...
      peak_memory_mb <- as.numeric(bench_result_preprocess$mem_alloc) / 1024^2
    }
    ...
    if (!requireNamespace("bench", quietly = TRUE)) {
      ...
      # Update peak memory if this step used more
      callr_memory <- as.numeric(bench_result_callr$mem_alloc) / 1024^2
      if(is.na(peak_memory_mb) || callr_memory > peak_memory_mb) {
        peak_memory_mb <- callr_memory
      }
    }
