...
#'            MARS (Multivariate Adaptive Regression Splines) via earth package
run_scPred_earth_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("bench", quietly = TRUE)) {
    ...
    seurat_train <- trainModel(seurat_train, model = "earth", allowParallel = TRUE)
  ...
  return(list(
    ...
    warning(paste("scPred (earth) error:", e$message))
    return(default_return())
  })
}

# For backward compatibility
run_scPred_earth <- run_scPred_earth_function
