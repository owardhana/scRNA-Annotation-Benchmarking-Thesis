...
try:
    ...
    # Initialize model (using Blood tissue as default for most benchmarks)
    model = DeepSortClassifier(
        species='human',
        tissue='Blood',
        dense_dim=400,
        hidden_dim=200,
        gpu_id=-1,  # Use CPU (RTX 5060 sm_120 incompatible with PyTorch 1.6.0)
        n_layers=1,              # 1 layer prevents GNN over-smoothing
        random_seed=1,
        n_epochs=50,
        num_neighbors=100,       # Limit neighbor sampling (None=all nodes causes over-smoothing + 13hr runtime)
    ...
    # Save peak memory to metadata file
...
'''
            ...
            # Load peak memory from metadata file
            metadata_path = os.path.join(temp_dir, "metadata.pkl")
            peak_memory_mb = None
            if os.path.exists(metadata_path):
                with open(metadata_path, 'rb') as f:
                    metadata = pickle.load(f)
                    peak_memory_mb = metadata.get('peak_memory_mb', None)
