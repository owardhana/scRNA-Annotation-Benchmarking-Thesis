...
run_CIPR_function <- function(seurat_train, seurat_test, markers) {
  ...
  # Step 1: Use Ground_Truth_Celltype as cluster groupings
  ...
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    ...
    custom_ref_expr <- tryCatch({
      avgexp <- AverageExpression(seurat_train, assays = "RNA", group.by = "Ground_Truth_Celltype", verbose = FALSE)$RNA
    ...
    # group.by = "Ground_Truth_Celltype" ensures column names are already cell type names
    ref_columns <- setdiff(colnames(custom_ref_expr), "gene")
    custom_ref_annot <- data.frame(
      short_name          = ref_columns,
      long_name           = ref_columns,
      reference_cell_type = ref_columns,
