...
run_SCINA_database_function <- function(seurat_train, seurat_test, markers) {
  ...
  convert_markers_to_scina <- function(marker_df, markers_per_type = 20) {
    # Validate input
    ...
    for(ct in cell_types) {
      ...
      # DATABASE MODE: No statistical filtering — CellMarker markers are curated
      # and arrive with synthetic stats (avg_log2FC=1.0, p_val_adj=0, pct.1=1.0).
      # The original FindAllMarkers filters (avg_log2FC >= 0.5, p_val_adj < 0.05,
      # pct.1 >= 0.15) are not meaningful here. Gene overlap with the expression
      # matrix (lines below) handles removing absent genes.
      filtered <- ct_markers
      ...
      # Filter to valid gene symbols only — CellMarker DB contains protein names
      # (e.g., "Ki67", "Musashi-1"), Greek characters ("IFN-γ", "β-catenin"),
      # and descriptive text that will never match Seurat gene symbols.
      valid_gene <- grepl("^[A-Za-z][A-Za-z0-9._-]*$", filtered$gene) &
                    nchar(trimws(filtered$gene)) <= 20 &
                    !grepl("[^\x01-\x7F]", filtered$gene, perl = TRUE)
      filtered <- filtered[valid_gene, ]
  ...
  exp_data <- tryCatch({
    d <- GetAssayData(seurat_test, assay = "RNA", layer = "data")
    if (nrow(d) == 0) d <- GetAssayData(seurat_test, assay = "RNA", layer = "counts")
    as.matrix(d)
  }, error = function(e) {
    tryCatch({
      d <- GetAssayData(seurat_test, assay = "RNA", slot = "data")  # Seurat v4
      if (nrow(d) == 0) d <- GetAssayData(seurat_test, assay = "RNA", slot = "counts")
      as.matrix(d)
    }, error = function(e2) {
      as.matrix(GetAssayData(seurat_test, assay = "RNA", slot = "counts"))
    })
  })
  cat(sprintf("Expression data: %d genes x %d cells\n", nrow(exp_data), ncol(exp_data)))
  
  # Convert markers to SCINA signatures format
  signatures <- convert_markers_to_scina(markers, markers_per_type = 20)
  ...
  # Remove signatures with too few genes (need at least 2 for SCINA in database mode;
  # CellMarker entries are curated, so even 2 genes can be informative)
  ...
  if(is.null(signatures_filtered) || length(signatures_filtered) == 0) {
    warning("No valid signatures found for SCINA (all had < 2 genes)")
    return(list(
  ...
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    ...
    results <- tryCatch({
      SCINA(exp_data,
            ...
            # rm_overlap = FALSE in database mode: CellMarker supplies hundreds of
            # overlapping tissue cell-type signatures, so removing shared genes
            # (the default TRUE) empties every signature and SCINA aborts with
            # "'a' must have dims > 0". Curated DB markers are kept as-is.
            rm_overlap = FALSE,
            allow_unknown = FALSE
      )
    }, error = function(e) {
    ...
    peakRAM_result <- peakRAM::peakRAM({
      # peakRAM evaluates in parent.frame() so <- assigns in calling scope
      results <- tryCatch({
        SCINA(
          exp_data,
          signatures_filtered,
          ...
          convergence_rate = 0.99,
          sensitivity_cutoff = 1,
          # rm_overlap = FALSE in database mode (see note above): overlapping
          # CellMarker signatures would otherwise be emptied, aborting SCINA.
          rm_overlap = FALSE,
          allow_unknown = FALSE
        )
      }, error = function(e) {
...
run_SCINA_database <- run_SCINA_database_function