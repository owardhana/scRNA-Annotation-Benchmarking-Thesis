...
run_ScInfeR_database_function <- function(seurat_train, seurat_test, markers) {
  ...
  n_test_clusters <- length(unique(seurat_test$Ground_Truth_Cluster))
  ...
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    ...
    scinfer_result <- tryCatch({
      predict_celltype_scRNA_seurat(
        ...
        group_annt = seurat_test$Ground_Truth_Cluster,
...
run_ScInfeR_database <- run_ScInfeR_database_function
