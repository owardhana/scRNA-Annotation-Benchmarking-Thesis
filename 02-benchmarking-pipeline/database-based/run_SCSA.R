...
run_SCSA_database_function <- function(seurat_train, seurat_test, markers,
                              scsa_py = "database-based/SCSA.py",
                              db_path = "database-based/whole.db",
                              scsa_env = "scsa_env",
                              keep_files = TRUE) {
  ...
  tryCatch({
    ...
    # Use markers directly as user reference DB (-M) — DB markers are pre-curated
    train_markers_filtered <- markers
...
# Step 2: Find markers for test clusters
.scsa_find_test_markers <- function(seurat_test) {
  ...
  cat(sprintf("SCSA: %d marker genes across %d test clusters\n",
              nrow(raw_markers), length(unique(raw_markers$cluster))))
  return(raw_markers)
}
...
run_SCSA_database <- run_SCSA_database_function
