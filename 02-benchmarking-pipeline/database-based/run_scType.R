...
run_scType_database_function <- function(seurat_train, seurat_test, markers) {
  ...
  # Use Ground_Truth_Celltype as cluster assignments (run_sctype reads seurat_clusters internally)
  cat("Using Ground_Truth_Celltype as cluster assignments for scType...\n")
  seurat_test@meta.data[["seurat_clusters"]] <- as.factor(seurat_test@meta.data[["Ground_Truth_Celltype"]])
  ...
  results <- tryCatch({
    ...
    fallback_result <- tryCatch({
      ...
      # Take top 20 markers per cell type (standardized)
      ...
      if (!requireNamespace("peakRAM", quietly = TRUE)) {
        ...
        runtime_secs <<- as.numeric(difftime(Sys.time(), start_time2, units = "secs"))
      } else {
        library(peakRAM)
        peakRAM_result_fallback <- peakRAM::peakRAM({
          es.max <- sctype_score(scRNAseqData = as.matrix(GetAssayData(seurat_test, assay="RNA", layer="scale.data")),
                                 scaled = TRUE,
                                 gs = gs_positive,
                                 gene_names_to_uppercase = FALSE)
        })
        runtime_secs <<- peakRAM_result_fallback$Elapsed_Time_sec[1]
        peak_system_memory_mb <<- peakRAM_result_fallback$Peak_RAM_Used_MiB[1]
      }
...
run_scType_database <- run_scType_database_function