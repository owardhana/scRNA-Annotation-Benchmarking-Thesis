...
run_scPred_rf_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    ...
    seurat_train <- trainModel(seurat_train, model = "rf", allowParallel = TRUE)
  ...
  return(list(
    ...
    warning(paste("scPred (rf) error:", e$message))
    return(default_return())
  })
}

# For backward compatibility
run_scPred_rf <- run_scPred_rf_function