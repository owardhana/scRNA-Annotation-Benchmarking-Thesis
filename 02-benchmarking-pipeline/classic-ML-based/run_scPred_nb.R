...
run_scPred_nb_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    ...
    seurat_train <- trainModel(seurat_train, model = "nb", allowParallel = TRUE)
  ...
  return(list(
    ...
    warning(paste("scPred (nb) error:", e$message))
    return(default_return())
  })
}

# For backward compatibility
run_scPred_nb <- run_scPred_nb_function
