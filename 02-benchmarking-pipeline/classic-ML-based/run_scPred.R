...
run_scPred_function <- function(seurat_train, seurat_test, markers) {
  ...
  tryCatch({
    ...
    if (!requireNamespace("peakRAM", quietly = TRUE)) {
      ...
      seurat_train <- trainModel(seurat_train, allowParallel = TRUE)
      ...
      peakRAM_result <- peakRAM::peakRAM(
        {
          ...
          # 2. Train machine learning model
          seurat_train <- trainModel(seurat_train, model = "svmRadial", allowParallel = TRUE)
    ...
    warning(paste("scPred error:", e$message))
    return(default_return())
  })
}

# For backward compatibility
run_scPred <- run_scPred_function