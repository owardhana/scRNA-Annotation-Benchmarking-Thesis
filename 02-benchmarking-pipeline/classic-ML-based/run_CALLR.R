...
run_CALLR_function <- function(seurat_train, seurat_test, markers) {
  ...
  tryCatch({
    ...
    filtered_markers <- prepare_markers(markers, top_n = 20)
    ...
    marker_matrix <- matrix("", nrow = length(cell_types), ncol = 20)  # top 20 markers per type
    ...
    for(i in 1:length(cell_types)) {
      ...
      n_available <- min(ncol(marker_matrix), length(available_markers))
    ...
    u_param <- 0.5  # Weight for LR signal in MBO update (original paper default)
