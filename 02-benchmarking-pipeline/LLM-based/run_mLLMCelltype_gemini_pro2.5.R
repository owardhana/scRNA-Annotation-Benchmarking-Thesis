# run_mLLMCelltype_gemini_pro2.5.R
# Single-model variant using Gemini 2.5 Pro

run_mLLMCelltype_gemini_pro2.5_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    ...
    annotation_results <- tryCatch({
      cat("Calling annotate_cell_types with Gemini 2.5 Pro...\n")
      ...
      warning("mLLMCelltype Gemini 2.5 Pro annotation failed: ", e$message)
    ...
    if (is.null(annotation_results)) {
      warning("mLLMCelltype Gemini 2.5 Pro failed - returning default predictions")
      return(default_return())
    }
...
run_mLLMCelltype_gemini_pro2.5 <- run_mLLMCelltype_gemini_pro2.5_function
