# run_mLLMCelltype_llama4_maverick.R
# Single-model variant using Llama 4 Maverick via OpenRouter

run_mLLMCelltype_llama4_maverick_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    ...
    annotation_results <- tryCatch({
      cat("Calling annotate_cell_types with Llama 4 Maverick...\n")
      ...
      warning("mLLMCelltype Llama 4 Maverick annotation failed: ", e$message)
    ...
    if (is.null(annotation_results)) {
      warning("mLLMCelltype Llama 4 Maverick failed - returning default predictions")
      return(default_return())
    }
...
run_mLLMCelltype_llama4_maverick <- run_mLLMCelltype_llama4_maverick_function
