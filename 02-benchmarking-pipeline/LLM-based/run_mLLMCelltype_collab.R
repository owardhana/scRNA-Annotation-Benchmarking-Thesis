...
run_mLLMCelltype_collab_function <- function(seurat_train, seurat_test, markers) {
  ...
  if (nchar(api_keys$openrouter) == 0 && nchar(api_keys$openai) == 0) {
    warning("mLLMCelltype_collab: no API keys found (OPENROUTER_API_KEY or OPENAI_API_KEY required)")
  ...
  if (!"Ground_Truth_Cluster" %in% colnames(seurat_test@meta.data)) {
    warning("mLLMCelltype_collab: Ground_Truth_Cluster column missing from seurat_test — cannot build cell data frame")
    return(default_return())
  }
  if ("Ground_Truth_Cluster" %in% colnames(seurat_train@meta.data)) {
    ...
    if (any(as.character(markers$cluster) %in% mapping_df$Ground_Truth_Celltype)) {
      ...
      if (na_marker_rows > 0) {
        warning("mLLMCelltype_collab: ", na_marker_rows, " marker rows have no cluster mapping and will be removed")
        markers <- markers[!is.na(markers$cluster), ]
      }
      if (nrow(markers) == 0) {
        warning("mLLMCelltype_collab: no markers remain after cluster remapping")
        return(default_return())
      }
    }
  }
  ...
  run_consensus <- function() {
    tryCatch({
      cat("mLLMCelltype_collab: calling interactive_consensus_annotation with 3 models...\n")
      ...
      warning("mLLMCelltype_collab consensus annotation failed: ", e$message)
      NULL
    })
  }
  ...
  if (is.null(consensus_results)) {
    warning("mLLMCelltype_collab: consensus failed — returning default predictions")
    return(default_return())
  }
  ...
  cat(sprintf("\n=== [mLLMCelltype_collab] Prediction mapping (%d clusters) ===\n", nrow(norm_df)))
  if (exists("mapping_df")) {
    cluster_to_ct <- setNames(as.character(mapping_df$Ground_Truth_Celltype),
  ...
  consensus_details <- if (!is.null(consensus_results$initial_results)) {
    ...
    warning("mLLMCelltype_collab: initial_results missing from consensus output — entropy confidence unavailable")
    NULL
  }
...
run_mLLMCelltype_collab <- run_mLLMCelltype_collab_function
