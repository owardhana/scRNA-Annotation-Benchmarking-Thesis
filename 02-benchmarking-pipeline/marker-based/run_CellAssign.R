...
run_CellAssign_function <- function(seurat_train, seurat_test, markers) {
  ...
  tryCatch({
    ...
    # Create marker list from filtered markers (already limited to top 10)
    marker_list <- list()
    cell_types <- unique(filtered_markers$cluster)

    for(ct in cell_types) {
      ct_markers <- filtered_markers[filtered_markers$cluster == ct, "gene"]
      if(length(ct_markers) > 0) {
        marker_list[[ct]] <- as.character(ct_markers)
      }
    }
