...
#' 
#' Purpose: Computes accuracy, precision, recall, F1, and macro F1 scores
#' Inputs: 
#'   - predicted: Vector of predicted cell types
#'   - true_labels: Vector of ground truth cell types  
#' Outputs: List with overall_accuracy, macro_f1, per_class_metrics dataframe
...
#'   - fold_peak_memories: Vector of per-fold peak memory usage (MB)
#'   - tool_name: Name of the tool
#' Outputs: List with pooled_metrics, fold_variation, runtime_stats, memory_stats, detailed_results
...
aggregate_tool_results <- function(all_predictions, all_true_labels, all_confidence_scores,
                                  all_cell_ids, fold_metrics, fold_runtimes, fold_peak_memories, tool_name) {
  ...
  # Memory statistics (filter valid peak memories)
  valid_peak_memories <- fold_peak_memories[!is.na(fold_peak_memories)]
  memory_stats <- list(
    mean_mb = mean(valid_peak_memories, na.rm = TRUE),
    std_mb = sd(valid_peak_memories, na.rm = TRUE),
    max_mb = max(valid_peak_memories, na.rm = TRUE),
    fold_peak_memories = valid_peak_memories
  )
...
run_single_tool_cv <- function(seurat_obj, tool_function, folds, tool_name) {
  ...
  for(i in 1:length(folds)) {
    ...
    tryCatch({
      ...
      fold_runtimes[i] <- runtime

      # Extract peak memory if available
      peak_memory <- fold_result$peak_memory_mb
      if(is.null(peak_memory)) peak_memory <- NA
      fold_peak_memories[i] <- peak_memory

      cat(sprintf("    Completed in %.2f seconds, accuracy: %.2f%%, memory: %.1f MB\n",
                  runtime, fold_metrics[[i]]$overall_accuracy * 100,
                  ifelse(is.na(peak_memory), 0, peak_memory)))
      
    }, error = function(e) {
      warning(sprintf("Tool %s failed on fold %d: %s", tool_name, i, e$message))
      fold_metrics[[i]] <- NULL
      fold_runtimes[i] <- NA
      fold_peak_memories[i] <- NA
  ...
  return(aggregate_tool_results(all_predictions, all_true_labels,
                               ...
                               fold_metrics, fold_runtimes, fold_peak_memories, tool_name))
}
...
aggregate_all_tools <- function(all_tool_results) {
  ...
  comparison_df <- data.frame(
    ...
    runtime_std = numeric(),
    peak_memory_mean_mb = numeric(),
    peak_memory_std_mb = numeric(),
    peak_memory_max_mb = numeric(),
  ...
  # Sort by pooled accuracy (primary metric)
  comparison_df <- comparison_df[order(-comparison_df$pooled_accuracy), ]
