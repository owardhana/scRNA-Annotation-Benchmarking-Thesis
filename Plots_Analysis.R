...
PLATFORM_SHIFTS <- c(
  "CB-1" = "droplet_to_plate",    "CB-2" = "within_droplet",
  "CB-3" = "droplet_to_plate",    "CB-4" = "within_droplet",
  "PX-1" = "droplet_to_plate",    "PX-2" = "droplet_to_plate",
  "PBMC-A" = "within_droplet",    "PBMC-B" = "within_droplet",
  "PBMC-C" = "droplet_to_nanowell"
)
...
load_benchmark_results <- function(base_dir, exclude_tools = NULL) {
  ...
  combined[["platform_shift"]] <- factor(PLATFORM_SHIFTS[combined$trial], levels = c("within_droplet", "droplet_to_nanowell", "droplet_to_plate"))
...
df <- load_benchmark_results(
  base_dir = BASE_DIR,
  exclude_tools = c('scInfeR', 'scPred_adaboost', "scGAD", "scDeepSort", "scibetR", "CIForm", "CAMLU", "scAnnotatR")
)
...
df_full <- expand_grid(tool = all_tools, trial = all_trials) %>%
  ...
  mutate(
    ...
    platform_shift = factor(PLATFORM_SHIFTS[trial], levels = c("within_droplet", "droplet_to_nanowell", "droplet_to_plate"))
  )
...
make_heatmap <- function(metric_col, title, is_unassigned=FALSE) {
  ...
  plot_data <- plot_data %>%
    mutate(
      val_text = if_else(is.na(!!sym(metric_col)), "NA", sprintf("%.2f", !!sym(metric_col))),
      val_text = case_when(
        trial == "PBMC-B" & !is.na(!!sym(metric_col)) ~ paste0(val_text, "*"),
        trial %in% c("PX-1", "PX-2") & !is.na(!!sym(metric_col)) ~ paste0(val_text, "\u2020"),
        TRUE ~ val_text
      )
    )
  ...
  if (is_unassigned) {
    fill_scale <- scale_fill_gradient2(
      ...
      midpoint = 0.5, limits = c(0, 1), name = "Mean Score", na.value = "grey80"
    )
  } else {
    fill_scale <- scale_fill_gradient2(
  ...
  p_annot <- ggplot(annot_data, aes(x = trial, y = "Shift", fill = platform_shift)) +
    geom_tile(colour = "white", linewidth = 0.4) +
    facet_grid(dummy ~ block, scales = "free", space = "free") +
    scale_fill_manual(
      values = c("within_droplet" = "#d5d8dc", "droplet_to_nanowell" = "#85929e", "droplet_to_plate" = "#2e4053"),
      name = "Platform Shift"
    ) +
    theme_void() +
    theme(
  ...
  p_main <- ggplot(plot_data, aes(x = trial, y = tool, fill = !!sym(metric_col))) +
    geom_tile(colour = "white", linewidth = 0.4) +
    geom_text(aes(label = val_text), size = 3.5, colour = "white", fontface = "bold") +
    ...
    theme(
      ...
      plot.margin   = margin(t = 5, b = 10, l = 10, r = 10)
    )
...
p6 <- ggplot(px_data, aes(x = trial, y = kappa_mean, colour = tool_category)) +
  ...
  geom_text_repel(data = filter(px_data, kappa_mean > 0.4), aes(label = tool),
...
slope_calcs <- robustness_data %>%
  ...
  mutate(drop = within_droplet - droplet_to_plate) %>%
  filter(!is.na(drop))
...
p7 <- ggplot(robustness_plot_data, aes(x = platform_shift, y = mean_kappa, group = tool, colour = tool_category)) +
  ...
  geom_text_repel(data = filter(robustness_plot_data, platform_shift == "droplet_to_plate" & highlight != "Normal"),
                  aes(label = tool), nudge_x = 0.2, direction = "y", hjust = 0, size = 2.8,
...
p11 <- ggplot(runtime_data, aes(x = mean_rt, y = mean_kappa, colour = tool_category)) +
  ...
  geom_text_repel(data = pareto_fronts, aes(label = tool), size = 2.8,
...
make_block_cd <- function(block_id) {
  ...
  p <- ggplot(mr_df, aes(x = mean_rank, y = 0)) +
    ...
    geom_text_repel(aes(label = tool, colour = tool_category),
...
ggsave("Plot1_heatmap_kappa.png",            p1,  width = 10, height = 12, dpi = 300, path = PLOT_DIR)
ggsave("Plot2_heatmap_f1.png",               p2,  width = 10, height = 12, dpi = 300, path = PLOT_DIR)
ggsave("Plot3_heatmap_unassigned.png",       p3,  width = 10, height = 12, dpi = 300, path = PLOT_DIR)
ggsave("Plot4_cellbench_direction.png",      p4,  width = 10, height = 5,  dpi = 180, path = PLOT_DIR)
ggsave("Plot5_pbmc_gradient.png",            p5,  width = 10, height = 5,  dpi = 180, path = PLOT_DIR)
ggsave("Plot6_pancreas_pair.png",            p6,  width = 8,  height = 5,  dpi = 180, path = PLOT_DIR)
ggsave("Plot7_batch_robustness_slope.png",   p7,  width = 10, height = 6,  dpi = 180, path = PLOT_DIR)
ggsave("Plot8_biology_vs_platform.png",      p8,  width = 10, height = 8,  dpi = 180, path = PLOT_DIR)
ggsave("Plot9_category_boxplots.png",        p9,  width = 13, height = 10, dpi = 180, path = PLOT_DIR)
write.csv(cat_ranking, file.path(PLOT_DIR, "category_table.csv"), row.names = FALSE)
ggsave("Plot10_category_robustness.png",     p10, width = 9,  height = 5,  dpi = 180, path = PLOT_DIR)
ggsave("Plot11_runtime_scatter.png",         p11, width = 14, height = 5,  dpi = 180, path = PLOT_DIR)
ggsave("Plot12_critical_difference.png",     p12, width = 14, height = 12, dpi = 180, path = PLOT_DIR)
ggsave("Plot13_category_shift_barplot.png",  p13, width = 10, height = 6,  dpi = 180, path = PLOT_DIR)
